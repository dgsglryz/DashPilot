...
class ReportGeneratorService
{
    ...
    private function buildPdfHtml(Site $site, int $templateId, string $startDate, string $endDate): string
    {
        ...
        $styles = $this->buildPdfStyles();
        $header = $this->buildPdfHeader($templateName, $site->name, $startDate, $endDate);
        $clientInfo = $this->buildPdfClientInfo($client, $site);
        $metrics = $this->buildPdfMetrics($site);
        $summary = $this->buildPdfSummary($site, $startDate, $endDate);
...
HTML;
    ...
    /**
     * Build CSS styles for PDF report.
     *
     * @return string
     */
    private function buildPdfStyles(): string
    {
        return <<<'CSS'
        body {
...
CSS;
    ...
    /**
     * Build header section for PDF report.
     *
     ...
     * @return string
     */
    private function buildPdfHeader(string $templateName, string $siteName, string $startDate, string $endDate): string
    {
        return <<<HTML
    <div class="header">
        <h1>DashPilot Operations Report</h1>
        <p>{$templateName} - {$siteName}</p>
...
HTML;
    ...
    /**
     * Build client information section for PDF report.
     *
     * @param Client $client
     * @param Site $site
     *
     * @return string
     */
    private function buildPdfClientInfo(Client $client, Site $site): string
    {
        return <<<HTML
    <div class="section">
...
HTML;
    ...
    /**
     * Build performance metrics table for PDF report.
     *
     * @param Site $site
     *
     * @return string
     */
    private function buildPdfMetrics(Site $site): string
    {
        $backups = random_int(20, 40);
        $scans = random_int(4, 10);
...
HTML;
    ...
    /**
     * Build summary section for PDF report.
     *
     * @param Site $site
     * @param string $startDate
     * @param string $endDate
     ...
     * @return string
     */
    private function buildPdfSummary(Site $site, string $startDate, string $endDate): string
    {
        $generatedAt = now()->format('F d, Y \a\t H:i');

        return <<<HTML
    <div class="section">
        <h2>Report Summary</h2>
        <p>This report was generated on {$generatedAt} and covers the period from {$startDate} to {$endDate}.</p>
...
HTML;
    ...
    /**
     * Build footer section for PDF report.
     *
     * @return string
     */
    private function buildPdfFooter(): string
    {
        $year = date('Y');
    ...
    <div class="footer">
        ...
        <p>© {$year} DashPilot. All rights reserved.</p>
    </div>
HTML;
    }
