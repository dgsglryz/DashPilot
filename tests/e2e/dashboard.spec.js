...
import { waitForPageReady } from "./helpers/wait.js";
...
test.describe("Dashboard", () => {
    ...
    test("should display dashboard with all stats cards", async ({ page }) => {
        ...
        await expect(
            page.locator(
                "text=/Site Monitoring|SEO Performance|Revenue Overview/i",
            ),
        ).toBeVisible();
    });
    ...
    test("should display current operations section", async ({ page }) => {
        ...
        await expect(
            page.locator("text=/Health Checks|Queue Workers|Cache Status/i"),
        ).toBeVisible();
    });
    ...
    test("should click on featured site and navigate to site detail", async ({
        page,
    }) => {
