...
test.describe('Settings', () => {
  ...
  test('should update email preferences', async ({ page }) => {
    ...
    if (await emailSection.isVisible()) {
      ...
      const emailCheckbox = page.locator('input[type="checkbox"][name*="email"], input[type="checkbox"][name*="notification"]').first();
      ...
      const submitButton = page.locator('button:has-text("Save Preferences"), button[type="submit"]').first();
      if (await submitButton.isVisible()) {
        await submitButton.click();
        await waitForSuccessMessage(page);
      }
  ...
  test('should add new webhook', async ({ page }) => {
    ...
    if (await webhooksSection.isVisible()) {
      ...
      const addButton = page.locator('button:has-text("Add Webhook"), a:has-text("Add Webhook"), [data-testid="add-webhook-button"]').first();
  ...
  test('should test webhook', async ({ page }) => {
    ...
    if (await webhooksSection.isVisible()) {
      ...
      const testButton = page.locator('button:has-text("Test"), [data-testid="test-webhook-button"]').first();
  ...
  test('should delete webhook', async ({ page }) => {
    ...
    if (await webhooksSection.isVisible()) {
      ...
      const deleteButton = page.locator('button:has-text("Delete"), [data-testid="delete-webhook-button"]').first();
  ...
  test('should update password', async ({ page }) => {
    ...
    if (await passwordSection.isVisible()) {
      ...
      const newPasswordInput = page.locator('input[name="password"], input[name="new_password"]');
      if (await newPasswordInput.isVisible()) {
        await newPasswordInput.fill('newpassword123');
      }

      const confirmPasswordInput = page.locator('input[name="password_confirmation"], input[name="confirm_password"]');
      if (await confirmPasswordInput.isVisible()) {
        await confirmPasswordInput.fill('newpassword123');
      }

      // Submit password form
      const submitButton = page.locator('button:has-text("Update Password"), button[type="submit"]').first();
      if (await submitButton.isVisible()) {
        await submitButton.click();
        await waitForSuccessMessage(page);
      }
  ...
  test('should test email notification', async ({ page }) => {
    ...
    if (await emailSection.isVisible()) {
      ...
      const testEmailButton = page.locator('button:has-text("Test Email"), [data-testid="test-email-button"]').first();
  ...
  test('should update monitoring settings', async ({ page }) => {
    ...
    const monitoringSection = page.locator('text=/Monitoring|Health Check/i');
    
    if (await monitoringSection.isVisible()) {
      // Update check interval if exists
      const intervalInput = page.locator('input[name="check_interval"], input[name="interval"]');
  ...
  test('should update alert thresholds', async ({ page }) => {
    ...
    const thresholdsSection = page.locator('text=/Thresholds|Alert Thresholds/i');
