...
test.describe('Authentication - Comprehensive Tests', () => {
  ...
  test('should show error with invalid password', async ({ page }) => {
    ...
    await page.waitForSelector('text=/These credentials do not match|Invalid credentials/i', { timeout: 5000 });
    await expect(page).toHaveURL(/\/login/);
  });
  ...
  test('should test remember me checkbox', async ({ page, context }) => {
    ...
    const hasRememberToken = cookies.some(c => c.name.includes('remember') || c.name.includes('remember_token'));
  ...
  test('should test password reset flow', async ({ page }) => {
    ...
    const forgotPasswordLink = page.locator('a:has-text("Forgot"), a[href*="password"]');
    ...
    if (await forgotPasswordLink.isVisible()) {
      ...
      if (await emailInput.isVisible()) {
        ...
        if (await submitButton.isVisible()) {
          ...
          await page.waitForSelector('text=/password reset|check your email/i', { timeout: 5000 });
        }
      }
    }
  });
